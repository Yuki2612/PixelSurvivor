...
public class Building {
    public static class DoorInfo {
        ...
        public DoorInfo(int x, int y, String side) { this.x = x; this.y = y; this.side = side; }
    }
    ...
    public Building(java.util.List<Rectangle> components, java.util.List<DoorInfo> doors) {
        ...
        for (Rectangle r : components) {
            ...
            // Mở rộng mái tối thiểu 80px để che phủ hoàn toàn lớp tường (64px) và tạo hiên (16px)
    ...
    public void update(Player player) {
        ...
        if (interiorArea.contains(playerCenterX, playerCenterY)) {
            roofAlpha -= fadeSpeed;
            if (roofAlpha < 0.1f) roofAlpha = 0.1f;
        } else {
            roofAlpha += fadeSpeed;
            if (roofAlpha > 1.0f) roofAlpha = 1.0f;
        }
    }
    ...
    public void renderFloor(Graphics2D g) {
        ...
        for (DoorInfo door : doors) {
            ...
            else pRect = new Rectangle(px + 64, py, pSize, pSize); // Side E
